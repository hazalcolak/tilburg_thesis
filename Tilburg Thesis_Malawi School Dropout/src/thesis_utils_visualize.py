...
def plot_aupr(models, ax=None):
    ...
    for i, model in enumerate(models):
        if model['fpr'] is not None:
            ...
            ax.plot(model['recall_thr'], model['precision_thr'], lw=lw,
                    label='{} AUC = {:0.2f}'.format(model['name'], model['aupr']))
...
def get_rank_order(x):
    ...
    ascending = x.name in ('cross_entropy')
...
def display_model_comparison(comp_models, 
    ...
    n_axes = sum([show_roc, show_cm, show_aupr])
    if n_axes > 0:
    ...
    # Display score table
    disp_metrics = ['accuracy', 'recall', 'precision', 'f1', 'cross_entropy', 'roc_auc', 'cohen_kappa','aupr']
    disp_types = {'max': ['accuracy', 'recall', 'precision', 'f1', 'roc_auc', 'cohen_kappa','aupr'], 
                  'min': ['cross_entropy']}
    met_df = []
